    ...
    <section className="bg-surface-container-low rounded-xl p-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {courses.map((course, index) => (
          <div key={index} className="card-content-safe bg-white p-6 rounded-xl text-center flex flex-col items-center shadow-sm">
            ...
            <h4 className="text-wrap-safe font-bold font-headline">{course.title}</h4>
            <p className="text-wrap-safe text-xs text-on-surface-variant mt-2">{course.desc}</p>
          </div>
        ))}
      </div>
    </section>
